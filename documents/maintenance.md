...
Keep keyboard access to the content through the localized skip link. Scroll positions must follow actual element sizes and CSS anchor spacing; dialog closing must follow the animation's completion. Keep intentional design dimensions in CSS and name repeated behavior limits. Do not patch layout with guessed pixel offsets, duplicated animation timers, or clipped interactive content. Short viewports must allow the whole dialog or sidebar pane to scroll when its controls no longer fit.
...
For a framework change, check the homepage, Learn overview, a long article, a namespace, a type/member page and an enum. Include wide, narrow and short landscape viewports, plus the effective small viewport produced by browser zoom. Check the bounds and keyboard reachability of controls as well as document overflow, the skip link, scrollable code/tables, menu exclusivity, search return/retry behavior, and member navigation. Verify interrupted and disabled animations. Translation fixtures must remain outside the repository and cover successful switching, explicit-language priority, unchanged fragments, and invalid dictionary fallback. Retain the original English text and code unless the task explicitly includes content edits.
