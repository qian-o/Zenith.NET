...
Render the tutorial image on two indexed triangles. This workload adds ImageSharp texture loading, mip levels, a sampler, and bindless resource handles. Start from [Project Setup](../project-setup.md), then choose **Textured Quad** from the tutorial selector.
...
Before the draw, each texture mip is transitioned to `Sampled`. The drawable is transitioned to `ColorAttachment`, cleared, and used by a render pass containing one indexed draw.
