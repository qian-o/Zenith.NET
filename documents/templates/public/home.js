...
export function initializeHomeScene() {
    ...
    const lightDirection = normalize([-0.55, -0.8, 1]);
    const halfDirection = normalize([lightDirection[0], lightDirection[1], lightDirection[2] + 1]);
    const dot = (a, b) => a.reduce((sum, value, i) => sum + value * b[i], 0);
    const center = t => [
        (1.2 + 0.38 * Math.cos(3 * t)) * Math.cos(2 * t),
        (1.2 + 0.38 * Math.cos(3 * t)) * Math.sin(2 * t),
        0.58 * Math.sin(3 * t)
    ];
    const point = (step, side) => {
        ...
        return c.map((v, i) => v + 0.26 * (normal[i] * Math.cos(angle) + binormal[i] * Math.sin(angle)));
    };
    for (let i = 0; i < steps; i++) {
        for (let j = 0; j < sides; j++) {
            ...
            const normal = normalize(
                cross(
                    vertices[3].map((v, index) => v - vertices[0][index]),
                    vertices[1].map((v, index) => v - vertices[0][index])
                )
            );
    ...
    function draw() {
        ...
        for (const { vertices, normal, depth } of transformed) {
            ...
            if (mode === 'surface') {
                ...
                context.strokeStyle = `rgba(200,211,226,${0.1 + (depth + 1.7) * 0.12})`;
    ...
    function tick(time) {
        ...
        if (rotating) {
            ...
            speed = Math.min(0.16, speed + elapsed * 0.2);
    ...
    canvas.addEventListener('pointermove', event => {
        ...
        pitch += (event.clientY - pointer.y) * 0.008;
