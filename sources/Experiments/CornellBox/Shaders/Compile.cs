...
Pass[] passes =
[
    new("PathTracingPass", [new("CSMain", "PathTracing.slang")]),
    new("DenoisePass", [new("TemporalMain", "Denoise.slang"), new("AtrousMain", "Denoise.slang")]),
    new("TonemapPass", [new("CSMain", "Tonemap.slang")])
];
...
{
    ShaderDesc shaderDesc = ZenithCompiler.CompileFromFile(graphicsApi, Path.Combine(shadersDirectory, shader.FileName), shader.EntryPoint);
...
static string CreateSkeleton(string passName, GraphicsApi[] graphicsApis, ShaderSource[] shaders)
{
    ...
    builder.AppendLine("using Zenith.NET;");
    builder.AppendLine();
    builder.AppendLine("namespace CornellBox.Passes;");
    builder.AppendLine();
...
{
    return $"{graphicsApi}{shader.EntryPoint}";
}
...
{
    File.WriteAllText(path, NormalizeGeneratedText(text) + LineEnding, new UTF8Encoding(true));
...
{
    return Path.GetFullPath(Path.Combine(Path.GetDirectoryName(filePath)!, "..", "Assets", "Shaders"));
}
...
file readonly struct ShaderSource(string entryPoint, string fileName)
{
